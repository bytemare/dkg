...
func FrostGenerateZeroKnowledgeProof(
	...
	r := g.Base().Multiply(k)
	c := challenge(g, id, pubkey, r)
	mu := k.Add(secret.Copy().Multiply(c))
...
func FrostVerifyZeroKnowledgeProof(g group.Group, id uint64, pubkey *group.Element, proof *Signature) bool {
	c := challenge(g, id, pubkey, proof.R)
	rc := g.Base().
		Multiply(proof.Z).
		Subtract(pubkey.Copy().Multiply(c))
